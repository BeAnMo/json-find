...
const TESTS = [
  ...
  },
  {
    ...
    /**
     * both keys are nested to the same depth,
     * but because A appears first, the return value is '91281102'
     * A = ROOT.list.story[0].container[0].link.refId
     * B = ROOT.list.story[0].layout.bottom.container.refId
     * A contains multiple refIds, but object at .container[0] contains duplicate
     * instances of the "link" key. This means only the last of the duplicate
     * keys will be acknowledged.
     */
    expected: '91281102',
    msg: MESSAGE('refId, same depth, last key')
  },
  {
    test: 'notStrictEqual',
    actual: runActual('checkKey', 'refId'),
...
function readJSON(file) {
  return new Promise((success, failure) => {
    ...
    fs.readFile(currentPath, 'utf-8', (err, data) => {
      return err ? failure(err) : success(JSON.parse(data));
    });
  });
}
